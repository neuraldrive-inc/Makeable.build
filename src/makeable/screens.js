...
async function handlePartsPhoto(context, route, file) {
  ...
  try {
    ...
    const plan = await requestHardwarePlan({
      ...
      fetchImpl: apiFetchFor(context.window),
...
function renderCode(context, route) {
  ...
  outlet.innerHTML = screenFrame(`
    <article class="route-screen code-screen">
      ...
      <div class="code-layout">
        <section class="paper-panel firmware-panel firmware-panel--sealed" aria-labelledby="firmware-title">
          <div class="firmware-ready-mark" aria-hidden="true">
            ${icon("code-xml")}
            ${icon("sparkles")}
          </div>
          <h2 id="firmware-title">Your board software is ready</h2>
          <p>
            Makeable keeps the generated firmware inside this build, compiles it on the hosted
            ESP32 service, and loads it straight onto your connected board.
          </p>
          <ul class="firmware-ready-list">
            <li>${icon("check")} ESP32-family target selected automatically</li>
            <li>${icon("check")} Safety diagnostics included</li>
            <li>${icon("check")} No IDE, source-code copy, or download step</li>
          </ul>
          <aside class="sticky-note code-note">${escapeHtml(
            firmware.notes ||
              project.feasibility?.firmwareSpec?.behavior ||
              "Review the wiring, connect the board, and Makeable will handle the rest.",
          )}</aside>
        </section>
        ...
        <section class="board-panel" aria-labelledby="connect-title">
          ...
          <p class="configured-board">Board target: ESP32 family</p>
          <p class="hardware-status" data-hardware-status role="status" aria-live="polite">
            Checking the hosted ESP32 compiler…
          </p>
  ...
  outlet.querySelector("[data-flash-board]").addEventListener("click", () =>
    flashCurrentFirmware(context, route, firmware.sketch || "", configuredFqbn),
  );
    ...
    <article class="route-screen publish-screen">
      <header class="screen-heading publish-heading">
        <div>
          ...
          <p>Makeable will package your guide, parts list, and test results. Your board software stays sealed.</p>
        </div>
      </header>
            ...
            <ul>
              ${artifacts
                .map(
                  ({ path }) => `
                    <li>
                      ${icon(path === "README.md" ? "paperclip" : "github")}
      ...
      <div class="publish-ready-row" aria-label="Package contents ready">
        ...
        <p>Firmware source stays private inside Makeable.</p>
      </div>
...
async function publishFromScreen(context, route, form, validate) {
  ...
  if (!window.MAKEABLE_CONFIG?.hasGithubToken) {
    status.textContent =
      "GitHub publishing is temporarily unavailable on the Makeable server.";
    return;
  }
  const visibility = new FormData(form).get("visibility") || "public";
  button.disabled = true;
  button.textContent = "Packaging your project…";
  status.textContent = "Creating the repository and uploading four project notes files…";
  try {
      ...
      <section class="paper-panel success-package">
        ...
        <div class="success-project-details">
          ...
          <ul class="publish-results">
            ...
            <li>${icon("check")} Firmware stayed sealed</li>
          </ul>
...
async function refreshArduinoSetup(context) {
  ...
  try {
    const response = await apiFetchFor(context.window)("/api/esp32/status");
    ...
    if (
      ...
      statusNode.textContent =
        "Hosted ESP32 compiler ready. Connect your board when you’re ready to load it.";
      flashButton.disabled = false;
      return;
    }
    statusNode.textContent =
      status.message ||
      "The hosted ESP32 compiler is temporarily unavailable. Try again in a moment.";
    flashButton.disabled = true;
  } catch (error) {
    statusNode.textContent = `I couldn’t check the hosted ESP32 compiler: ${error.message}`;
    flashButton.disabled = true;
  }
}
...
async function flashCurrentFirmware(context, route, sketch, configuredFqbn) {
  ...
  try {
    ...
    const flashedFirmware = {
      ...(app.getProject().firmware || {}),
      sketch: result.sketch || app.getProject().firmware?.sketch || sketch,
    };
    await app.updateProject(
      "firmware",
      transitionFirmwareFlash(flashedFirmware, "success", {
...
async function toggleVoice(context, textarea) {
  ...
  try {
    const tokenResponse = await apiFetchFor(context.window)("/api/deepgram/token", {
      method: "POST",
...
function apiFetchFor(windowLike) {
  return windowLike.MAKEABLE_API_FETCH || windowLike.fetch.bind(windowLike);
}
