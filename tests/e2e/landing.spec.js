...
test("the public homepage explains Makeable with one Google waitlist action", async ({
  ...
  await expect(page).toHaveTitle("Build the thing in your head. · Makeable");
  await expect(
    page.getByRole("heading", {
      level: 1,
      name: "Build the thing in your head.",
    }),
  ).toBeVisible();
...
test("the 1536 by 1024 landing composition matches the approved reference proportions", async ({
  ...
  expect(metrics.headlineFontSize).toBeGreaterThanOrEqual(78);
  expect(metrics.headlineFontSize).toBeLessThanOrEqual(84);
...
test("public Google signup verifies through the server and confirms the waitlist", async ({
  ...
  await page.getByRole("button", { name: "Share Makeable" }).click();
  await expect
