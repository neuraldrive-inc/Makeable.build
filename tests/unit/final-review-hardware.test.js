...
test("the local server serves only the explicit Raymond UI surface", async () => {
  const serverSource = await readFile(path.join(root, "server.mjs"), "utf8");
  assert.match(serverSource, /const publicRootFiles = new Set/);
...
test("GitHub uploads fail closed without a user-bound publish capability", async () => {
  const response = await fetch(`${baseUrl}/api/github/upload-file`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      owner: "neuraldrive-inc",
      repo: "another-users-project",
      path: "README.md",
