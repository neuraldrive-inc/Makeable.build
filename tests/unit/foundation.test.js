...
test("the package and local server identify the product as Makeable", async () => {
  ...
  assert.equal(packageJson.name, "makeable-hardware-builder");
  assert.match(server, /Makeable running at/);
  assert.doesNotMatch(server, /GeckCo AI running at/);
});

test("the current backend discovers its pinned hosted ESP32 toolchain", async () => {
  const server = await read("server.mjs");

  assert.match(server, /env\.ARDUINO_CLI_PATH/);
  assert.match(server, /\.makeable\/toolchain\/bin\/arduino-cli/);
  assert.match(server, /supportedBoardSummary/);
});
