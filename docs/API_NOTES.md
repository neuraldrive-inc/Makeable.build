...
- `OPENAI_REASONING_EFFORT=high` and `OPENAI_SERVICE_TIER=priority` are enforced at the server proxy for every OpenAI request. Priority tier availability depends on the configured OpenAI project.
